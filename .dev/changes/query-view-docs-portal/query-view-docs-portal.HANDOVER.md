...
- 模板与案例只复制代码，不直接写入笔记。
- 核心智能体技能必须自包含，可与页面和人类文档工作并行；不要假定 `references/` 可被 SiYuan 原生 Agent 加载。
...
- `references/`、Skill 安装/调用和 SiYuan Agent/MCP 集成均为延后任务。

## File Reference Map

- `TARGET.SPEC.md` — 目标、范围、行为和验收标准。
- `graph.md` — 计划任务和依赖；下一个可启动工作是“定义文档结构与内容来源规则”或“编写核心智能体技能”。

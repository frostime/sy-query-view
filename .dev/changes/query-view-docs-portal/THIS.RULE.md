...
文档站主体、第一轮运行时反馈修复、核心 Skill、Skill 文档站接入和自动发布验证均已验收。当前无代码执行任务，等待用户在隔离 SiYuan 工作区完成最终运行时复测。`references/` 文件读取研究由用户明确延后；禁止在未获用户明确同意时操作用户数据库。

Skill 的安装、调用、启用状态展示及 SiYuan Agent/MCP 完整集成是延后工作。`references/` 当前不纳入核心 Skill；未来只有在实测目标 Agent 能访问其实际安装路径后才可加入，不得凭目录结构假定。

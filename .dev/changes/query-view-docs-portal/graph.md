...
**当前状态：** 已进入方案阶段；尚未开始任何生产代码任务。
**当前前沿：** “定义文档结构与内容来源规则”正在执行。
...
| 任务 | 独立交付物 | 依赖 | 状态 |
|---|---|---|---|
| 定义文档结构与内容来源规则 | 一份页面地图和内容规则：`docs/zh_CN/`、`docs/en_US/` 对应页面，`docs/TERM.md`，README 生成顺序，案例引用，单一帮助入口，以及离线静态材料规则。 | 用户已确认进入方案阶段。 | 执行中；见 `nodes/define-doc-structure/` |
| 整理人类文档 | 建立对应的中英文 docs 页面和术语表，吸收现有 README 教程、案例导览和 API 导览；生成并提交中英文 README，提供同步检查。 | 定义文档结构与内容来源规则。 | 等待 |
| 开发文档站 GUI | 在现有自定义 Tab 方案上用原生 TypeScript/DOM 实现桌面文档站，使用 Lute、SiYuan CSS class/变量，具备侧边栏、页面导航、语言选择和复制操作。 | 定义文档结构与内容来源规则。 | 等待 |
...
当前执行任务：`nodes/define-doc-structure/TASK-NODE.SPEC.md`。

...
data = {
  ...
  "domains": [
    ...
    {"key": "n6", "title": "N6 — 代码扫清（扫描/类型/注释）+ API 诚实化", "issues": [
      ...
      {"id": "I-64", "title": "时间函数默认 hms=true（非 yyyyMMdd）；thisWeek 从周日始", "prio": "P1", "status": "done", "ev": "WB", "disposition": "文档化", "note": "已补：JSDoc 标注 default: true（用户拍板披露）"},
      {"id": "I-65", "title": "dv.render() 会写嵌入块（updateEmbedBlock）副作用", "prio": "P0", "status": "open", "ev": "WB", "disposition": "文档化", "note": "data-view.ts:1423；行为确认，不轻易改；⚠ KNOWN_NOTES 已披露"},
      {"id": "I-66", "title": "useState 会话/块属性延迟持久化未文档化", "prio": "P1", "status": "done", "ev": "WB", "disposition": "文档化", "note": "已补：@note 写入时机（dispose 时写块属性，非实时）"},
      {"id": "I-67", "title": "details content 字符串直拼 innerHTML（非 markdown）；open=true", "prio": "P1", "status": "done", "ev": "WB", "disposition": "文档化", "note": "已披露：JSDoc + ⚠ KNOWN_NOTES"},
      {"id": "I-68", "title": "addmd 实支持 KaTeX（文档称不支持）", "prio": "P1", "status": "open", "ev": "WB", "disposition": "修文档", "note": "components.ts:1352-1374；行为正确，文档错；第三批（修实现决策）前不做"},
      {"id": "I-69", "title": "echartsTree layout:'radial' 参数被忽略（硬编码 orthogonal）", "prio": "P1", "status": "open", "ev": "WB", "disposition": "修实现/文档化", "note": "data-view.ts:1151-1158；第三批，用户拍板"},
      {"id": "I-70", "title": "addcolumns flex:[1,1,2] 未按列生效", "prio": "P1", "status": "open", "ev": "WB", "disposition": "修实现/文档化", "note": "data-view.ts:686-690；第三批，用户拍板"},
      {"id": "I-71", "title": "关系图/嵌入组件默认值缺（flowchart+LR、breadcrumb、zoom1）；graph roam 默认 false", "prio": "P2", "status": "wontfix", "ev": "WB", "disposition": "文档化", "note": "用户判定（2026-08-24）：影响观感不影响正确性，不披露"},
      {"id": "I-72", "title": "graph 渲染 mutate 调用者链接对象（删 source/target）", "prio": "P2", "status": "open", "ev": "WB", "disposition": "修实现/文档化", "note": "data-view.ts:1308-1318；第三批，用户拍板"},
      {"id": "I-74", "title": "Query.request 是内核请求非任意 HTTP；Query.gpt 才是外部 fetch", "prio": "P0", "status": "done", "ev": "WB", "disposition": "修文档", "note": "已补：request/gpt @note 安全边界说明"},
      {"id": "I-75", "title": "IDataView 手写桩只有 render()，与真实 class 脱节", "prio": "P1", "status": "done", "ev": "WB", "disposition": "规范/废弃", "note": "已删：ICustomView.use 改 dv?: any + 注释说明运行时为 DataView 实例；教程/模板同步"},
      {"id": "I-76", "title": "IWrappedList 假泛型：interface <T> 但实现硬编码 Block", "prio": "P2", "status": "done", "ev": "scan", "disposition": "修类型", "note": "已改默认泛型 IWrappedList<T = Block>（元素语义保留，成员保持 Block 系与实现一致）；pick 深度类型问题归 I-61/第三批"},
      {"id": "I-77", "title": "内部方法未标 private：isValidViewContainer/view 已标", "prio": "P2", "status": "done", "ev": "scan", "disposition": "修类型", "note": "两个均 private+@internal（view 用户拍板加标）；已验证从 d.ts 消失"},
      {"id": "I-78", "title": "details 无 JSDoc（register 组件中唯一无注释）", "prio": "P1", "status": "done", "ev": "scan", "disposition": "补注释", "note": "已完成：行为真相（innerHTML 直拼/open=true）入 JSDoc"},
      {"id": "I-79", "title": "deprecated 参数名暴露内部实现（optionDeprecatedAsValMatch 等）", "prio": "P3", "status": "hang", "ev": "scan", "disposition": "待用户决策", "note": "独立节点（C-3）：用户决定保留 or 2.0 清理；2026-08-24 用户明确单独处理，不在 N6 本批做；对应 graph.yaml conditional_candidates C-3"},

...
Query 与 DataView 两级的别名（含自动生成的全小写别名，query.ts、data-view.ts）一经发布即可能被脚本依赖，
移除任何别名按第 2 节流程处理。

### 3.9 日期字符串与本地时间

Query 时间 API 中，8 位 `yyyyMMdd` 表示不含时间和时区的日历日期，14 位 `yyyyMMddHHmmss` 表示精确到秒的
本地日期时间。转换为 JavaScript `Date` 时均按运行环境的本地时区解释，其中 8 位日期映射到该日期的起始时刻；
不得按 UTC 解析。日记查询负责把 Date 或 14 位输入归一化为 8 位日期，任务更新时间边界负责归一化为 14 位日期时间。

日期输出的正式格式参数为 `'date' | 'datetime'`。旧布尔值在兼容期内保持 `false → 'date'`、
`true → 'datetime'` 的映射，并按第 2 节的废弃通道警告。
